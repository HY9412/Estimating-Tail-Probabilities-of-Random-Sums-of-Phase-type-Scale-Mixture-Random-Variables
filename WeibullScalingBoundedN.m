...
for j=1:length(gammaU)
    ...
    for i=1:N
        ...
        Wb1=wblrnd(a,b);  
        Y_AtsComIS(i)=R*gamcdf(val/Wb1,nEr,1/lambda1,'upper')-(R-(n1*p1+1))*gamcdf(gammaU(j)/Wb1,nEr,1/lambda1,'upper');
    end
...
for j=1:length(gammaU)
    ...
    for i=1:N
        ...
        theta1=1+1./log((wblcdf(gammaU(j)*lambda1/nEr,a,b,'upper')));
        aa=1-rand^(1/(1-theta1));
        S32(i)=wblinv(aa,a,b);                               %Sample from the hazard rate twisting S
        Y_AtsComIS2(i)=(R*gamcdf(val/S32(i),nEr,1/lambda1,'upper')-(R-(n1*p1+1))*gamcdf(gammaU(j)/S32(i),nEr,1/lambda1,'upper'))*exp(-theta1*(S32(i)/a)^b)*(1/(1-theta1));
    end   
...
for j=1:length(gammaU)
    ...
    for i=1:N
        ...
        Y_Ats(i)=M*wblcdf(val1/Yts,a,b,'upper')-(M-(n1*p1+1)).*wblcdf(gammaU(j)/Yts,a,b,'upper');   
    end
    ell_onPH(j) = mean(Y_Ats);
    RE_onPH(j) = std(Y_Ats)/sqrt(N)/ell_onPH(j);
    LE_onPH(j)=abs(log(var(Y_Ats))/log(ell_onPH(j)^2));
end
...
for j=1:length(gammaU)
    ...
    for i=1:N
        ...
        lambdaIS = (n1*p1)*(a*gamma(1+1/b))/gammaU(j);
        Yts=exprnd(1/lambdaIS);
        L = (1/lambdaIS)*(lambda1/(lambda1-lambdaIS))^nEr*gampdf(Yts,nEr,1/(lambda1-lambdaIS));
        Y_Ats2(i)=(M*wblcdf(val1/Yts,a,b,'upper')-(M-(n1*p1+1)).*wblcdf(gammaU(j)/Yts,a,b,'upper'))*L;
    end
...
for j=1:length(gammaU)
    ...
    for i=1:N
        ...
        lambdaIS2 = nEr*(n1*p1)*(a*gamma(1+1/b))/gammaU(j);
        YtsIS=gamrnd(nEr,1/lambdaIS2);
        L2 = (lambda1/lambdaIS2)^nEr*exp(-(lambda1-lambdaIS2)*YtsIS);
        Y_Ats2IS(i)=(M*wblcdf(val1IS/YtsIS,a,b,'upper')-(M-(n1*p1+1)).*wblcdf(gammaU(j)/YtsIS,a,b,'upper'))*L2;
    end
    ell_onPHIS2(j) = mean(Y_Ats2IS);
    RE_onPHIS2(j) = std(Y_Ats2IS)/sqrt(N)/ell_onPHIS2(j);
    LE_onPHIS2(j)=abs(log(var(Y_Ats2IS))/log(ell_onPHIS2(j)^2));
end
figure(1)
